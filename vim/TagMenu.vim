...
function! TagMenu(mode)
   "let ctagsProg = "!ctags --langmap=c:.c.h.hc.hm.hp.ht.hv -h .h.hc.hm.hp.ht.hv"
   let ctagsProg = "!ctags"
...
"   let ctagsProg = "!jtags -c -iF"
   ...
   "
   execute '9000amenu Ta&gs.&(Re)Generate\ Tags.&This\ File\ Only :call TagMenu("")<CR>'
   execute 'amenu Ta&gs.&(Re)Generate\ Tags.T&his\ File\ Only\ w/\ Submenus :call TagMenu("-s")<CR>'
   execute 'amenu Ta&gs.&(Re)Generate\ Tags.&Directory :call TagMenu("-d")<CR>'
   execute 'amenu Ta&gs.&(Re)Generate\ Tags.D&irectory\ w/\ Submenus :call TagMenu("-d-s")<CR>'
   execute 'amenu Ta&gs.&(Re)Generate\ Tags.&Recurse :call TagMenu("-r")<CR>'
   execute 'amenu Ta&gs.&(Re)Generate\ Tags.&Parse\ Only :call TagMenu("-p")<CR>'
